...
const AdminGamesPending = () => {
  ...
  const fetchPendingGames = async () => {
    try {
      setLoading(true);
      const response = await api.get('/games/?admin=true');  // Add query param
      const allGames = Array.isArray(response.data) ? response.data : response.data.results || [];
      const pendingGames = allGames.filter(game => game.status === 'pending');
  ...
  const handleApproveGame = async (gameId) => {
    try {
      ...
      fetchPendingGames(); // Refresh the list
    } catch (error) {
        ...
        <div className="games-table">
          ...
          {games.map(game => (
            <div key={game.id} className="table-row">
              ...
              <div>{game.developer_name || game.developer}</div>
              <div>${game.price}</div>
